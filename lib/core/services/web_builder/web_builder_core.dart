  ...
  try {
    ...
    if (!await webBuildersDir.exists()) {
      await webBuildersDir.create(recursive: true);
        ...
        final webBuildersPathCurrentProject = path.join(webBuildersDir.path, prjName);
    final webBuildersDirCurrentProject = Directory(webBuildersPathCurrentProject);
   ...
   final targetDir = await getTargetDirectory(folderUnderApp: "");
    ...
    if (!await webBuildersDir.exists()) {
      await webBuildersDir.create(recursive: true);
        ...
        final webBuildersPathCurrentProject = path.join(webBuildersDir.path, '$prjName-${DateTime.now()}');
    final webBuildersDirCurrentProject = Directory(webBuildersPathCurrentProject);
    if (!await webBuildersDirCurrentProject.exists()) {
      await webBuildersDirCurrentProject.create(recursive: true);
    }

...
final class TfTerminal extends ToolFunc {
        ...
        parametersSchema: const {
          ...
          'properties': {
            'command': {
              'type': 'string',
              'description':
                  'The shell command to execute (e.g., "ls -la", "cat file.txt", "sudo apt update").',
            },
          },
          'required': ['command'],
        },
  ...
  String get description =>
      'Execute Linux shell commands and return output. Supports all commands including sudo.';
  ...
  Future<_Ret?> run(_CallResp call, _Map args, OnToolLog log) async {
    ...
    if (command == null || command.isEmpty) {
      final error = ToolError.invalidInput(
        'command',
        suggestion:
            'Provide a valid shell command (e.g., "ls -la", "cat file.txt").',
      );
    ...
    try {
      ...
      if (result.exitCode != 0) {
        ...
        final error = ToolError.executionFailed(
          ...
          suggestion:
              'Review the error output above and adjust the command if needed.',
        );
        output.write('\n${error.toMessage()}');
      }
      ...
      final error = ToolError.executionFailed(
        ...
        suggestion:
            'Verify the command syntax and ensure all required tools are installed.',
      );
      log('Terminal Error: ${error.toMessage()}');
      return [ChatContent.text(error.toMessage())];
    }
